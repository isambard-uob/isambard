...
class HelixPair(Assembly):
    def __init__(self, aas=(10, 10), axis_distances=(-4.5, 4.5), z_shifts=(0, 0), phis=(0, 0), splays=(0, 0),
                 off_plane=(0, 0), build=True):
        """Generates a pair of helixes with an orientation defined relative to a central axis.
    ...
    def build(self):
        for i in range(2):
            self._molecules.append(
                self.make_helix(self.aas[i], self.axis_distances[i], self.z_shifts[i], self.phis[i], self.splays[i],
                                self.off_plane[i]))
        return
